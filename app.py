...
# --- CONFIGURACIÓN DE LA PÁGINA (CENTRADA Y LIMPIA) ---
st.set_page_config(page_title="Aliados de la Circularidad", page_icon="🌱", layout="centered")

# --- CSS: ESTILO MODERNO, FUENTE INTER Y TARJETAS LIMPIAS ---
st.markdown("""
    <style>
    ...
    /* Fuente y fondo general (Gris muy clarito) */
    html, body, [class*="css"] { font-family: 'Inter', sans-serif; }
    .stApp { background-color: #F8FAFC; }
    
    /* Contenedores con borde (estilo tarjeta blanca de tu imagen) */
    ...
    /* Tarjeta de Resultado (Elegante y Clara) */
    .resultado-card {
        background-color: #FFFFFF;
...
st.markdown("<h2 style='text-align: center; color: #0F172A; font-weight: 800;'>Aliados de la Circularidad ♻️</h2>", unsafe_allow_html=True)
st.markdown("<p style='text-align: center; color: #64748B; font-size: 1.1rem;'>Pequeños Detalles & Mina Las Bambas</p>", unsafe_allow_html=True)
st.write("---")
...
with tab1:
    ...
    with st.container(border=True):
        st.markdown("<h4 style='color: #1E293B;'>♻️ ¿Qué prendas vamos a reciclar?</h4>", unsafe_allow_html=True)
    ...
    if st.session_state.lista_prendas:
        ...
        if col_calc.button("🌱 Registrar y Calcular Impacto", type="primary", use_container_width=True):
            if not nombre:
                st.error("⚠️ Por favor, ingresa tu nombre en la parte superior.")
            else:
...
with tab2:
    ...
    if datos_supa:
        ...
        # MÉTRICAS ESTILO NATIVO DE STREAMLIT (Limpio y directo)
